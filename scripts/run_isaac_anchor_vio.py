#!/usr/bin/env python3
"""Validate configs and optionally start the standalone Isaac runtime."""
...
def _default_config_paths() -> dict[str, str]:
    return {
        ...
        "robot": "config/isaac/robot/ur5e_phone_head.yaml",
...
def main() -> int:
    ...
    for name, path in runtime.config.config_paths.items():
        writer.write_config_snapshot(name, load_isaac_yaml(path))
    manifest = build_run_manifest(
        repo_root=Path(__file__).resolve().parents[1],
        run_id=run_id,
        isaac_sim_version="available" if importlib.util.find_spec("isaacsim") else "unavailable_in_current_env",
        stage_usd_path=runtime.config.stage_path,
        robot_preset=runtime.config.robot_preset,
        anchor_tag_id=runtime.config.anchor_tag_id,
        noise_presets={
            "imu": str(load_isaac_yaml(args.imu_config).get("noise_preset", Path(args.imu_config).stem)),
            "actuation": str(load_isaac_yaml(args.actuation_config).get("name", Path(args.actuation_config).stem)),
        },
