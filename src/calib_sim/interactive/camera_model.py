...
def _normalized_rotation_degrees(video_mode: dict[str, Any]) -> int:
    raw_rotation = video_mode.get("encoded_rotation_degrees", video_mode.get("rotation_degrees", 0))
    rotation = int(raw_rotation) % 360
    if rotation not in (0, 90, 180, 270):
        raise ValueError(f"Unsupported encoded video rotation {raw_rotation!r}; expected 0, 90, 180, or 270 degrees.")
    return rotation


def _unrotated_output_size(output_width_px: int, output_height_px: int, rotation_degrees: int) -> tuple[int, int]:
    if rotation_degrees in (90, 270):
        return int(output_height_px), int(output_width_px)
    return int(output_width_px), int(output_height_px)


def _rotate_intrinsics_to_encoded_frame(
    *,
    fx_px: float,
    fy_px: float,
    cx_px: float,
    ...
    if rotation_degrees == 0:
        return fx_px, fy_px, cx_px, cy_px, skew_px
    if rotation_degrees == 90:
        return fy_px, fx_px, float(unrotated_height_px) - cy_px, cx_px, 0.0
    if rotation_degrees == 180:
        return fx_px, fy_px, float(unrotated_width_px) - cx_px, float(unrotated_height_px) - cy_px, -skew_px
    if rotation_degrees == 270:
        return fy_px, fx_px, cy_px, float(unrotated_width_px) - cx_px, 0.0
    raise ValueError(f"Unsupported encoded video rotation {rotation_degrees}.")
...
class PhoneCameraModel:
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "encoded_rotation_degrees": self.encoded_rotation_degrees,
    ...
    def project_camera_points(
        ...
        *,
        ...
        points_cv = np.column_stack((points[:, 0], -points[:, 1], points[:, 2])).astype(np.float64)
...
def load_phone_camera_model(
    ...
    *,
    ...
    rotation_degrees = _normalized_rotation_degrees(video_mode)
    unrotated_width_px, unrotated_height_px = _unrotated_output_size(
        final_width_px,
        final_height_px,
        rotation_degrees,
    )
    ...
    scale_x = float(unrotated_width_px) / crop_width_px
    scale_y = float(unrotated_height_px) / crop_height_px

    unrotated_fx_px = float(active_intrinsics["fx_px"]) * scale_x
    unrotated_fy_px = float(active_intrinsics["fy_px"]) * scale_y
    unrotated_cx_px = (float(active_intrinsics["cx_px"]) - crop_x_px) * scale_x
    unrotated_cy_px = (float(active_intrinsics["cy_px"]) - crop_y_px) * scale_y
    unrotated_skew_px = float(active_intrinsics.get("skew_px", 0.0)) * scale_x
    fx_px, fy_px, cx_px, cy_px, skew_px = _rotate_intrinsics_to_encoded_frame(
        fx_px=unrotated_fx_px,
        fy_px=unrotated_fy_px,
        cx_px=unrotated_cx_px,
        cy_px=unrotated_cy_px,
        skew_px=unrotated_skew_px,
        unrotated_width_px=unrotated_width_px,
        unrotated_height_px=unrotated_height_px,
    ...
    return PhoneCameraModel(
        ...
        encoded_rotation_degrees=rotation_degrees,
        fx_px=fx_px,
        fy_px=fy_px,
        cx_px=cx_px,
        cy_px=cy_px,
