...
def _native_pose_ready(detection: IsaacTagDetectionPacket) -> bool:
    measurement_source = str(getattr(detection, "measurement_source", "")).strip().lower()
    if measurement_source:
        measurement_pose_ready = measurement_source in {"native_detected", "retry_detected", "roi_recovered"}
    else:
        flags = dict(detection.visibility_flags)
        measurement_pose_ready = bool(flags.get("native_backend", False))
    flags = dict(detection.visibility_flags)
    pose_ready = bool(flags.get("pose_ready", False))
    return measurement_pose_ready and pose_ready and detection.pose_camera_tvec_m is not None and detection.pose_camera_rvec is not None
