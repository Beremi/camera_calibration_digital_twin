...
class IsaacRunWriter:
    ...
    def save_named_rgb_image(
        self,
        *,
        stream_name: str,
        frame_index: int,
        image_rgb: np.ndarray,
        stream_root: str = "observer_rgb",
    ) -> str:
        safe_stream_name = "".join(character if character.isalnum() or character in ("_", "-") else "_" for character in str(stream_name))
        target_dir = self.raw_dir / stream_root / safe_stream_name
        target_dir.mkdir(parents=True, exist_ok=True)
        path = target_dir / f"frame_{int(frame_index):06d}.png"
        image_bgr = cv2.cvtColor(np.asarray(image_rgb, dtype=np.uint8), cv2.COLOR_RGB2BGR)
        cv2.imwrite(str(path), image_bgr)
        return str(path.relative_to(self.run_dir))
    ...
    def write_observer_camera_frame(self, *, camera_name: str, payload: dict[str, Any], image_rgb: np.ndarray | None = None) -> None:
        packet = dict(payload)
        if image_rgb is not None and not packet.get("rgb_path"):
            packet["rgb_path"] = self.save_named_rgb_image(
                stream_name=str(camera_name),
                frame_index=int(packet.get("frame_index", 0)),
                image_rgb=image_rgb,
            )
        self._append_jsonl(self.raw_dir / "observer_camera_frames.jsonl", packet)
