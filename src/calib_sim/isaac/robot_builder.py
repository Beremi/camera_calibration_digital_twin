...
class RobotMountSpec:
    ...
    camera_mount_translation_m: tuple[float, float, float] = (0.0, 0.0, 0.0)
    camera_mount_rpy_deg: tuple[float, float, float] = (0.0, 0.0, 0.0)
    imu_mount_translation_m: tuple[float, float, float] = (0.0, 0.0, 0.0)
    imu_mount_rpy_deg: tuple[float, float, float] = (0.0, 0.0, 0.0)
...
class LiveRobotBinding:
    ...
    def joint_limits_rad(self) -> tuple[np.ndarray, np.ndarray] | None:
        lower: np.ndarray | None = None
        upper: np.ndarray | None = None
        try:
            if hasattr(self.articulation, "dof_properties"):
...
def robot_mount_spec_from_config(robot_config: dict[str, Any]) -> RobotMountSpec:
    return RobotMountSpec(
        ...
        camera_mount_translation_m=tuple(float(value) for value in robot_config.get("camera_mount_translation_m", (0.0, 0.0, 0.0))),
        camera_mount_rpy_deg=tuple(float(value) for value in robot_config.get("camera_mount_rpy_deg", (0.0, 0.0, 0.0))),
        imu_mount_translation_m=tuple(float(value) for value in robot_config.get("imu_mount_translation_m", (0.0, 0.0, 0.0))),
        imu_mount_rpy_deg=tuple(float(value) for value in robot_config.get("imu_mount_rpy_deg", (0.0, 0.0, 0.0))),
...
def build_robot_binding(*, world: Any, robot_config: dict[str, Any]) -> LiveRobotBinding:
    ...
    mount_specs = (
        (spec.camera_mount_prim_path, spec.camera_mount_translation_m, spec.camera_mount_rpy_deg),
        (spec.imu_mount_prim_path, spec.imu_mount_translation_m, spec.imu_mount_rpy_deg),
    )
    for mount_path, mount_translation_m, mount_rpy_deg in mount_specs:
        ...
        mount_xform = UsdGeom.Xformable(stage.GetPrimAtPath(mount_path))
        mount_xform.ClearXformOpOrder()
        mount_xform.AddTranslateOp().Set(tuple(float(value) for value in mount_translation_m))
        mount_orientation = euler_angles_to_quat(np.asarray(mount_rpy_deg, dtype=np.float64), degrees=True)
